import unittest
from application.Rules.SamuelRulesCaptcha01 import rules
...
class TestCaptcha01Methods(unittest.TestCase):
    rules = rules.SamuelRulesCaptcha01()
    validator = Validation.Validation()

    def test_captcha_resolve(self):
        request = {'captcha': self.rules.encode_base_64("tests/imagem_exemplo_captcha01.captcha")}
        string = self.rules.execute_resolve_captcha(request)
        self.assertEqual(string, 'ssn597')
    ...
    def test_error_base64_invalido(self):
        request = {'captcha': self.rules.encode_base_64("tests/imagem_exemplo_captcha01.captcha")[1:]}
